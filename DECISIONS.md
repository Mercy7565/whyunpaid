...
## The five-journey sweep

- **The masthead overflowed at 380px, which put the theme control off the right edge of a phone** — it measured 403px wide in a 380px viewport, so the toggle was unreachable on the narrowest common handset. Sizes and padding are tightened at that width and the row is allowed to wrap, because a control that scrolls out of view is a control nobody can use.
- **A copy button that fails silently is worse than one that admits it** — `navigator.clipboard.writeText` rejects on an insecure origin, a denied permission or an embedded frame, and the old handler swallowed the rejection, so the button simply did nothing. `copyText` now falls back to a selection and `execCommand`, and both copy buttons report the failure in words ("Use the address bar", "Select and copy") and through a live region.
- **The appeal letter's source rail had a press animation but was not a control** — a bulk edit had matched it. Removed: nothing should look pressable unless pressing it does something.
- **The test list was keyed by test name, and React was dropping rows** — a test that runs once per specimen policy carries the same name four times inside one file, so the expanded list rendered fewer rows than the count above it claimed. Keyed by position now: the list renders 159 of 159.
- **The evaluation order table said nothing about the policy actually selected** — it describes the engine, which is the same for all four specimens, so a reader looking at Specimen D saw a room rent stage for a policy that has no room rent clause and could not tell whether the stage had done nothing or done something invisible. Each stage now names the clauses this policy carries there, as buttons that reveal the wording, or says "no clause · deducts nothing".
- **Two measurement artefacts, recorded so they are not chased twice** — a contrast snippet that does not parse the `color(srgb 0 0 0)` form reports roughly 1.7:1 for text that actually measures 11:1, and `getBoundingClientRect` on a Browser pane that has been hidden returns zeroes for the whole document. Neither was a defect in the product; both looked exactly like one.

## Amber as text, which the stylesheet had already forbidden

- **Three numerals were drawn in amber on the light ground, at 1.5:1** — the palette comment in `globals.css` states plainly that amber "cannot be read as text on either ground ... so the payable figure never IS amber", and then three components did exactly that: the test count on the inspector, the months since inception on the appeal screen, and the payable figure in the mobile bar. Dark mode hid it, because amber on indigo measures 6:1; only the light steel ground exposed it, at 1.48:1 and 1.92:1.
- **The fix keeps the brand where it is legible and deepens it where it is not** — `--figure-good` and `--figure-alert` are the brand amber and a paprika tint in dark mode, and deep versions of the same two hues in light mode. They are now the only brand colours permitted as text anywhere, and the palette comment says so.
- **The mobile summary bar is only reachable below the medium breakpoint, and a desktop-width scan never sees it** — one of the three faults lived there. Any future contrast sweep has to run at a phone width as well, in both themes, and with a nil-payable scenario loaded so the alert branch is actually rendered.

## The command palette never took focus

- **Opening the palette left focus on whatever was behind it** — the effect scheduled `inputRef.current.focus()` inside `requestAnimationFrame`, and the callback did not reliably run. The dialog appeared, but `document.activeElement` stayed on a control behind the modal, so typing to filter did nothing and a keyboard reader was stranded outside an `aria-modal` dialog whose own footer promises "Up and down to move, Enter to run". Calling `focus()` by hand on the same input worked and stuck, which is what proved the call was never happening rather than being overridden.
- **It is a layout effect now** — that runs after the DOM is in place and before paint, which is exactly when the input is focusable, and it does not depend on a frame ever being produced.
- **The shortcut hint said the wrong thing off a Mac** — the button rendered a Command glyph for a shortcut a Windows reader presses with Control. It now renders "Ctrl K" unless the user agent is a Mac. The `aria-label` already said "Command or Control K" and is unchanged.
- **A live region counted in the plural regardless** — the waterfall's screen-reader summary read "1 deductions" on a claim blocked by a single clause.
